...
class User(db.Model):
    ...
    def serialize(self):
        return {
            ...
            "printable_registration_date": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.registration_date))
        }
...
def register():
    ...
    if request.method == "POST":
        try: 
            ...
            flash("Registrazione effettuata con successo, la tua password è: " + password)
            return redirect("/notes")
...
def login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            errors.append("Username o password errati")
...
def users():
    if "username" not in session:
        return redirect("/register")
    users = User.query.filter((User.username == "admin") | (User.username == session["username"])).all()
